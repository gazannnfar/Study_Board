...
AI слой находится в `apps/api/src/modules/ai/`. За единым интерфейсом `AiAssistantService` есть две реализации, переключаемые через `.env`:

- **`RuleBasedAiAssistant`** (`ai.service.ts`, `AI_PROVIDER=rule`, по умолчанию) — детерминированный движок без сети: название, описание, приоритет, теги, напоминание и PERT. Работает офлайн, безопасен для демо.
- **`LlmAiAssistant`** (`ai.llm.ts`, `AI_PROVIDER=llm`) — реальный LLM через **OpenAI-совместимый API**. Модель генерирует только текст (название, описание, summary, риски, зависимости, подзадачи, напоминание), а **PERT-числа остаются детерминированными** — модель их не считает. При отсутствии ключа или ошибке запроса автоматически откатывается на rule-based.

### Подключить реальную нейросеть (бесплатно через Groq)

1. Получить бесплатный ключ на <https://console.groq.com>.
2. В `apps/api/.env`:
...
Совместимо с любым OpenAI-style провайдером — достаточно сменить 3 строки (`AI_BASE_URL`/`AI_MODEL`/`AI_API_KEY`): OpenRouter, OpenAI, локальный Ollama (`http://localhost:11434/v1`). Примеры — в `apps/api/.env.example`.

Доступны две логические конфигурации (работают в обоих провайдерах):
